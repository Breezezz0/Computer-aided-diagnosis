...
dataset_valid = {}
dataset_test = {}
num_70 = int(len(dataset_emr.keys())*0.7) if int(len(dataset_emr.keys())*0.7) % 2 == 0 else int(len(dataset_emr.keys())*0.7)+1
num_90 = num_70 + int(len(dataset_emr.keys())*0.2)
for num,keys in enumerate(dataset_emr.keys()) :
    if num < num_70 :
        dataset_train[keys] = dataset_emr[keys]
    elif num >= num_70 and num < num_90 :
...
torch.save(model.state_dict(), "GPT2-model_0327.pth")
#繪製訓練過程
